...
export function UnsplashImageComponent({
  ...
  const imageAlt = alt || image.alt_description || 'Unsplash image';
  
  // Use useCase to determine optimal sizing if not provided
  const imageSizes = useCase ? getImageSizesForUseCase(useCase) : null;
  ...
  const optimizedImage = optimizeUnsplashImage(image, finalWidth, finalHeight);
  ...
  return (
    <div className="relative group">
      <Image
        ...
        blurDataURL={blurDataURL || optimizedImage.placeholder}
        quality={85}
      />
...
export function UnsplashImageWithHook({
  ...
  const { image, loading, error } = useRandomUnsplashImage(query, options);
...
export function OptimizedUnsplashImage({
  ...
  const { images, loading, error } = useOptimizedImagesForUseCase(category, useCase, count);
